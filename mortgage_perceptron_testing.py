...
for n in range(len(sample)):
    ...
    learning_rate=1#slow
    observed_output=expected_output-1
    vertices,observed_output=epoch(threshold)
    if observed_output!=expected_outcome:#false
        print('incorrect')
        if expected_outcome ==1:#positive
            fp+=1
        if expected_outcome ==-1:#negative
            fn+=1
        
    elif observed_output==expected_outcome:#true
        print('correct\n')
        if expected_outcome == 1:#positive
            tp+=1
        if expected_outcome ==-1:#negative
